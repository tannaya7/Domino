...
|---|---|---|
| **GitHub API (`GITHUB_TOKEN`)** | ✅ Verified live | A fine-grained PAT was configured in `server/.env` (git-ignored) and used for real `/analyze-repo` calls against `documenso/documenso`, `formbricks/formbricks`, and `stripe-samples/checkout-one-time-payments` during this session. Confirmed the token raised the anonymous 60 req/hour quota and that live analysis returns real vendor/file-graph data end-to-end through the actual UI. |
